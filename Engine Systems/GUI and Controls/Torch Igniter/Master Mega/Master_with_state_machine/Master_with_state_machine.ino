...
const int SEQUENCE_RUN_DURATION =
    1000; // Fixed run duration for sequence (500 ms)
...
int currentRunDuration =
    runDuration; // Current run duration (sequence or standalone)
...
const unsigned long PRESSURE_UPDATE_INTERVAL =
    10; // Update pressure every 10 ms
unsigned long sequenceStartTime =
    0; // To track when the sequence starts for timestamps
...
void setup() {
  ...
  Serial.println("Commands: 'a'-'p' for SSRs, 'startSeq' to start sequence, "
                 "'start dwell off duration' for spark only.");
...
void runStateMachine() {
  if (!sequenceRunning)
    return; // Only run if sequence is active
  ...
  case PREP:
    ...
    if (currentTime - stateStartTime >= 1000) {
      ...
      Serial.println(
          "PRESSURIZED: Oxygen Tank, Nitrogen Tank, Oxygen Vent, Fuel Vent ON");
    }
    break;
  ...
  case PRESSURIZED:
    ...
    if (currentTime - stateStartTime >= 3000) {
      ...
      Serial.println("TEST: Oxygen Tank, Nitrogen Tank, Oxygen Vent, Fuel "
                     "Vent, Oxygen Line, Fuel Line ON, Igniting...");
    }
    break;

  case TEST:
    // Oxygen Tank, Nitrogen Tank, Oxygen Vent, Fuel Vent, Oxygen Line, Fuel
    // Line on
    digitalWrite(OX_TANK_PIN, HIGH);
    digitalWrite(NITROGEN_TANK_PIN, HIGH);
    digitalWrite(OX_VENT_PIN, HIGH);
  ...
  case SPARK_TEST_OFF:
    if (currentTime - stateStartTime >= sparkOffTime) {
      ...
      if (currentTime - sparkStartTime >= currentRunDuration) {
        ...
        currentState = PURGE;
        stateStartTime = currentTime;
        Serial.println("PURGE: Oxygen Tank, Nitrogen Tank, Oxygen Vent, Fuel "
                       "Vent, Nitrogen Line ON");
      } else {
        // Continue sparking
        currentState = SPARK_TEST_DWELL;
        stateStartTime = currentTime;
  ...
  case PURGE:
    ...
    if (currentTime - stateStartTime >= 15000) {
      ...
      stateStartTime = currentTime;
      Serial.println("PRESSURIZED_2: Oxygen Tank, Nitrogen Tank, Oxygen Vent, "
                     "Fuel Vent ON");
    }
    break;
...
void runStandaloneSpark() {
  if (!standaloneSparkRunning)
    return; // Only run if standalone spark is active
...
void handleSerialCommands() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command.length() == 1) {
      ...
      command.trim();
      command.toLowerCase();
      command.toCharArray(cmdBuf, sizeof(cmdBuf));
      
      isSequence = parseArgs(cmdBuf);
    }
    if (isSequence) {
      ...
      if (!sequenceRunning && !standaloneSparkRunning) {
        ...
        sequenceStartTime =
            millis(); // Record sequence start time for timestamps
...
bool parseArgs(char* command) {

  char *argv[12];
  int argc = 0;

  // Using strtok_r to tokenize command into char* using whitespace as a
    // delimitter
  char *save;
  ...
  if (argc > 0) {  
    ...
    Serial.println("Error: No command provided.");
    return false;
  }
  ...
  for (int i = 1; i < argc; i++) {
    ...
    // transitionType & TEST_DWELL_TO_TEST_OFF || transitionType &
    // TEST_OFF_TO_PURGE; Sequencing mode
    if (isSequence) {
      if (transitionType != NUMBER_OF_TRANSITIONS) {
        if (i + 1 < argc) {
          ...
          Serial.println("Error: No " + String(transitionType) +
                         ...
                         sequenceTransitionTimes[transitionType]);
        }
      } else {
    ...
    // Standalone mode
      ...
      if (strcmp(argv[i], "--sparkdwelltime") == 0) {
        if (i + 1 < argc)
          sparkDwellTime = atoi(argv[i + 1]);
        else {
          Serial.println("Error: No dwell time provided. Using default of " +
                         String(sparkDwellTime));
        }
      }
      if (strcmp(argv[i], "--sparkdwelltime") == 0) {
        if (i + 1 < argc)
...
//  StartSeq --prepToPressurized 3000 --pressurizedToTest 15000 --testToPurge
//  1000 --purgeToPressurized2 1000 --pressurized2ToIdle 1000
TransitionTypes getTransitionType(const char *flagName) {
  if (strcmp(flagName, "--preptopressurized") == 0)
    return PREP_TO_PRESSURIZED;
  else if (strcmp(flagName, "--pressurizedtotest") == 0)
    return PRESSURIZED_TO_TEST;
  else if (strcmp(flagName, "--testtopurge") == 0)
    return TEST_TO_PURGE;
  else if (strcmp(flagName, "--purgetopressurized2") == 0)
    return PURGE_TO_PRESSURIZED_2;
  else if (strcmp(flagName, "--pressurized2toidle") == 0)
    return PRESSURIZED_2_TO_IDLE;
  else
